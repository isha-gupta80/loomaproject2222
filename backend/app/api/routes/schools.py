...
from app.services.schools import add_school, delete_school_by_id, get_school_stats, get_schoool_by_id, list_schools, update_school, update_school_status
...
@router.get("")
async def get_schools(stats: bool = False, province: str | None = None, search: str | None = None):
    if stats:
...
@router.delete("/{id}")
async def delete_school(id: PydanticObjectId):
    await delete_school_by_id(id)
    return {"detail": "success"}

@router.patch("/{id}/status")
async def update_status(id: PydanticObjectId, status_str: SchoolUpdateStatus):
...
@router.put("/{id}")
async def update(id: PydanticObjectId, data: SchoolUpdate):
